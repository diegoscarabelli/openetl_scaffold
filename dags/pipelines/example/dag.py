...
This file lives under `dags/pipelines/<name>/dag.py`, which Astro CLI and
native Airflow discover by default (both scan `dags/` recursively and add it
to PYTHONPATH, so `from lib.xxx import yyy` resolves without any extra setup).
"""
